...
if (!isset($_SESSION['user'])) {
    ...
    $query = $conn->prepare("SELECT * FROM pontos_turisticos WHERE id = ?");
    $query->bindParam(1,$_GET['id']);
    $query->execute();

    $response = $query->fetch();
    $id_ponto = $response['id'];
    $nome_ponto = $response['nome_ponto'];
...
<div class="container cad">
	<h1 id="titulo_cadastro"> Atualizar Ponto </h1>
    <div class="form">
	<form method="POST" action="Controller/action_atualizar_ponto.php" class="form" enctype="multipart/form-data">
        <input type="text" class="id_ponto" name="id_ponto" value="<?=$id_ponto?>">
        <div class="form-group">
            <label for="text">Nome do Ponto:</label>
            <input type="text" class="form-control" value="<?=$nome_ponto?>" name="nome_ponto">
        </div>

        <div class="form-group">
            <label for="text">Logradouro:</label>
            <input type="text" class="form-control" value="<?=$logradouro?>" name="logradouro">
        </div>
...
<!--         <div class="form-group">
        ...
        <div class="form-group">
            ...
            <textarea type="text" class="form-control" value="<?=$descricao?>" name="descricao"></textarea>
        </div>
